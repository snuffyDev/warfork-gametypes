...
Cvar kota_king_team_size( "g_kota_king_team_size", "2", 0 );
...
CondLimiter kotaPlayerNumLimiter( 10000 );
...
int[] killCount( maxClients );
int[] damageCount( maxClients );
...
// the purpose is to move the kota_king_team_size players from TEAM_BETA that have the most damage dealt
// to the TEAM_ALPHA team, or if TEAM_ALPHA has any kings that deal less damage than any of the kota_king_team_size players from TEAM_BETA the king will move to the TEAM_BETA team
//
void KOTA_endRound()
{

	Team@ team_alpha = @G_GetTeam( TEAM_ALPHA );
	Team@ team_beta = @G_GetTeam( TEAM_BETA );
	...
	int alpha_count = 0;
	int beta_count = 0;
	int new_alpha_count = 0;

	Entity@[] alpha_kings( kota_king_team_size.integer );
	Entity@[] beta_kings( kota_king_team_size.integer );

	// find the kings

	// find the kings from TEAM_ALPHA and sort them by damage dealt
	for ( i = 0; @team_alpha.ent( i ) != null; i++ )
	{
		@ent = @team_alpha.ent( i );

		// sort the kings by damage dealt
		for ( j = 0; j < kota_king_team_size.integer; j++ )
		{
			// if the king is null or the current king has dealt more damage the current ent
			if ( alpha_kings[ j ] == null || alpha_kings[ j ].client.stats.totalDamageGiven > ent.client.stats.totalDamageGiven )
			{
				@alpha_kings[ j ] = @ent;
				break;
			}
	...
	// find the kings from TEAM_BETA and sort them by damage dealt
	for ( i = 0; @team_beta.ent( i ) != null; i++ )
	{
		@ent = @team_beta.ent( i );
		for ( j = 0; j < kota_king_team_size.integer; j++ )
		{
			if ( beta_kings[ j ] == null || beta_kings[ j ].client.stats.totalDamageGiven > ent.client.stats.totalDamageGiven )
			{
				@beta_kings[ j ] = @ent;
				break;
			}
	...
	// move the kings
	for ( i = 0; i < beta_kings.length() - 1; i++ )
	{
		for ( j = 0; j < kota_king_team_size.integer - 1; j++ )
		{
		if ( @alpha_kings[ j ] != null && @beta_kings[ i ] != null )
		{
			if ( new_alpha_count >= kota_king_team_size.integer )
				break;
			int alphaDamage = 0;

			// avoid division by zero
			if ( damageCount[ alpha_kings[ j ].client.playerNum ] > 0 )
				alphaDamage = alpha_kings[j].client.stats.totalDamageGiven / damageCount[ alpha_kings[ j ].client.playerNum ];
			else
				alphaDamage = alpha_kings[j].client.stats.totalDamageGiven;

			int betaDamage = 0;

			// avoid division by zero
			if ( damageCount[ beta_kings[ i ].client.playerNum ] > 0 )
				betaDamage = beta_kings[i].client.stats.totalDamageGiven / damageCount[ beta_kings[ i ].client.playerNum ];
			else
				betaDamage = beta_kings[i].client.stats.totalDamageGiven;

			if ( alphaDamage >= betaDamage )
			{
				G_PrintMsg( null, beta_kings[i].client.name + " is now a king! (replacing: "+ alpha_kings[j].client.name + ")\n" );
				alpha_kings[ j ].client.team = TEAM_BETA;
				beta_kings[ i ].client.team = TEAM_ALPHA;
				Entity@ temp = @alpha_kings[ j ];
				@alpha_kings[ j ] = @beta_kings[ i ];
				@beta_kings[ i ] = @temp;
				new_alpha_count++;
			}
		}
	...
	// make sure Alpha team has only kota_king_team_size players
	for ( i = 0; i < kota_king_team_size.integer; i++ )
	{
		if ( @alpha_kings[ i ] != null )
		{
			G_PrintMsg( null, alpha_kings[ i ].client.name + " is no longer king! (King limit reached)\n" );
			alpha_kings[ i ].client.team = TEAM_BETA;
			alpha_count--;
		}
	}

	Team@ team;
	// reset the damage dealt for all Players
	for ( i = TEAM_PLAYERS; i < GS_MAX_TEAMS; i++ )
	{
		@team = @G_GetTeam( i );
...
class cCARound
{
	...
	void playerKilled( Entity @target, Entity @attacker, Entity @inflictor )
	{
		...
		if ( this.state == CA_ROUNDSTATE_PREROUND || this.state == CA_ROUNDSTATE_ROUND )
		{
			if ( this.numRounds > 1 && @target != null && @target.client != null )
			{
				WIPEOUT_setRespawnDelay( target.client, false );
			}

			// if the target is on TEAM_ALPHA and they are
		}
...
{
	G_PrintMsg( client.getEnt(), "debug: "+cmdString + "\nJoining team: " + argsString + "\n" + "info: " + argsString.getToken( 1 ) + "\n" );
	...
	{
		// only allow joining the BETA team if you're a spectator, otherwise don't allow joining
		if ( client.team == TEAM_SPECTATOR )
		{
			if ( argc > 1 )
			{
				if ( argsString.getToken( 1 ) == "beta" )
				{
	...
	for ( t = TEAM_ALPHA; t < GS_MAX_TEAMS; t++ )
	{
		...
		for ( i = 0; @team.ent( i ) != null; i++ )
		{
			...
			else
			{
				...
				entry = "&p " + playerID + " " + player_respawn_delay_str + " " + playerID + " " + ent.client.clanName + " "
						+ damageCount[ent.client.playerNum] + " " + killCount[ent.client.playerNum] + " "
						+ ent.client.ping + " " + ( ent.client.isReady() ? "1" : "0" ) + " ";
			}
...
void GT_ScoreEvent( Client @client, const String &in score_event, const String &in args )
{
	if ( score_event == "dmg" )
	{
		if ( match.getState() == MATCH_STATE_PLAYTIME )
		{
			...
			if ( @client != null )
			damageCount[client.playerNum] += client.stats.totalDamageGiven - damageCount[client.playerNum];
	...
	else if ( score_event == "kill" )
	{
		...
		if ( match.getState() == MATCH_STATE_PLAYTIME )
		{
			GT_updateScore( client );
			if ( @client != null )
			killCount[client.playerNum] = client.stats.frags;
		}
	}
...
void GT_ThinkRules()
{
	...
	if ( kotaPlayerNumLimiter.check() )
	{
		kotaPlayerNumLimiter.inc();
		if ( kota_king_team_size.string == kota_king_team_size.defaultString )
		{
			kota_king_team_size.set( floor( playerCount / 3 ) );
		}

		// check if the team names are set, if not, set them
		for ( int i = TEAM_ALPHA; i < GS_MAX_TEAMS; i++ )
		{
...
{
	gametype.title = "King of the Arena";
	...
	if ( gametype.isInstagib )
	{
		...
		G_ConfigString( CS_SCB_PLAYERTAB_TITLES, "AVATAR \u00A0 Name Clan Damage Respawn Ping R" );
	}
	else
	{
		G_ConfigString( CS_SCB_PLAYERTAB_LAYOUT, "%a l1 %s 16 %n 112 %s 52 %i 52 %i 52 %l 48 %r l1" );
		G_ConfigString( CS_SCB_PLAYERTAB_TITLES, "AVATAR \u00A0 Name Clan Damage Frags Ping R" );
	}
