...
String getBlessedMsg()
{
    int rnd = rand() % 12;
    switch( rnd )
    {
        case 0: return "i am so shit :<";
        case 1: return "haha";
        case 2: return "doh";
        case 3: return "aw";
        case 4: return "ez";
        case 5: return "nice lg";
        case 6: return "from behind :<";
        case 7: return "and again with the lg";
        case 8: return "i can tell yer good";
        case 9: return "how to play? am new";
        case 10: return "yer shit";
        case 11: return "<_<";
        case 12: return ">_>";
        default: return "aw";
    }
...
void sayPublicChat(Client @client)
{
    Entity @ent;
    Team @team;
    String msg = getBlessedMsg();
...
class cCARound
{
        ...
        case CA_ROUNDSTATE_POSTROUND:
        {
            ...
            if ( count_alpha > count_beta )
            {
                ...
                if ( !gametype.isInstagib && count_alpha == 1 ) // he's the last man standing. Drop a bonus
                {
                    ...
                    {
                        lastManStanding.client.addAward( S_COLOR_GREEN + "i can tell yer good!" );
                        // ch :
                        if( alpha_oneVS > ONEVS_AWARD_COUNT )
                        	// lastManStanding.client.addMetaAward( "Last Man Standing" );
                        	lastManStanding.client.addAward( "i can tell yer good!" );
            ...
            {
                G_CenterPrintMsg( null, "yer both shit!" );
            }
        }
        break;
        ...
        if ( this.state == CA_ROUNDSTATE_ROUND )
        {
			...
			if( this.minuteLeft > 0 )
			{
				...
				if ( caTimelimit1v1 != 0 && ( caTimelimit1v1 * 1000 ) == left )
				{
					...
					{
						G_CenterPrintMsg( null, "you got this many seconds left: " + caTimelimit1v1 );
					}
					else
					{
						uint minutes;
						...
						if( seconds == 0 )
						{
							...
							if(minutes == 1) {
                                G_CenterPrintMsg( null, "you got this many minutes left: " + caTimelimit1v1 );
							} else {
                                G_CenterPrintMsg( null, "you got this many minutes left: " + caTimelimit1v1 );
						...
						else
						{
                            ...
                            G_CenterPrintMsg( null, "you got this many minutes left: " + minutes + ":"+ seconds );
						}
					}
				}
				...
				{
					G_CenterPrintMsg( null , S_COLOR_RED + 'yer outta time!');
					this.newRoundState( this.state + 1 );
				}
			}
    ...
    void playerKilled( Entity @target, Entity @attacker, Entity @inflictor )
    {
        ...
        {
            if (attacker.client.weapon == WEAP_LASERGUN)
            {
                G_PrintMsg( attacker, "nice lg"+"\n" );
                attacker.client.armor = 0;
                attacker.health = 0;

            }

            sayPublicChat(@target.client);
			if ( gametype.isInstagib )
			{
				G_PrintMsg( target, "haha " + attacker.client.name + " just fragged you"+"\n" );
			}
			else
			{
				// report remaining health/armor of the killer
				G_PrintMsg( target, "am dead by " + attacker.client.name + " (health: " + rint( attacker.health ) + ", armor: " + rint( attacker.client.armor ) + ")\n" );
			}
            ...
            {
                G_PrintMsg( null, "1v1 <_<\n" );
                attacker.client.addAward( "1v1! <_<" );
                ...
                for ( int j = 0; @team.ent( j ) != null; j++ )
                {
                    ...
                    ent.client.addAward( S_COLOR_ORANGE + "1v1! <_<" );
                    break;
                }
            ...
            {
                attacker.client.addAward( "1v" + targetCount + "! yer on yer own" );

                // console print for the team
                @team = @G_GetTeam( attacker.team );
                for ( int j = 0; @team.ent( j ) != null; j++ )
                {
                    G_PrintMsg( team.ent( j ), "1v" + targetCount + "! " + attacker.client.name + ", yer on yer own!\n" );
                }
            ...
            else if ( attackerCount > 1 && targetCount == 1 )
            {
                ...
                for ( int j = 0; @team.ent( j ) != null; j++ )
                {
                    ...
                    ent.client.addAward( "1v" + attackerCount + "! yer on your own!" );
                    @survivor = @ent;
                    break;
                }
                ...
                {
                    @ent = @team.ent( j );
                    G_PrintMsg( ent, "1v" + attackerCount + "! " + survivor.client.name + ", yer on yer!\n" );
                }
