...
// Class for the player, to store the player's state
// and other useful information
// The player should have a 'think' function that is called each frame and client & ent pointers
// the player will float to the top of the map at the start of the game, and when the player's health decreases - so does the player's float height
class HighAsfPlayer
{
    Entity@ ent;
    Client@ client;
    ...
    void think()
    {
        ...
        if ( ent.health > 40 )
        {
            ...
            ent.mass = 40 * (1 - ent.health / ent.maxHealth);
...
HighAsfPlayer[] highAsfPlayers( maxClients );
...
bool GT_Command( Client @client, const String &cmdString, const String &argsString, int argc )
{
    ...
    {
        if ( argsString == "allow_collision" )
        {
            String value = argsString.getToken( 1 );
            if ( value != "0" && value != "1" )
            {
                G_PrintMsg( client.getEnt(), "Invalid value for allow_collision. Valid values are 0 or 1.\n" );
                return false;
            }
            return true;
        }
    ...
    else if ( cmdString == "callvotepassed" )
    {
        ...
        {
            String value = argsString.getToken( 1 );
            g_allow_collision.set( value.toInt() );
            G_PrintMsg( null, "Allow collision set to " + value + ".\n" );
            return true;
        }
    }
    return false;
...
void GT_ScoreEvent( Client @client, const String &score_event, const String &args )
{
    ...
    else if ( score_event == "disconnect" )
    {
        if ( @client != null )
        {
         @highAsfPlayers[client.playerNum].client = null;
         @highAsfPlayers[client.playerNum].ent = null;
    ...
    else if ( score_event == "connect" )
    {
        if ( @client != null )
        {
            @highAsfPlayers[client.playerNum].client = @client;
            @highAsfPlayers[client.playerNum].ent = @client.getEnt();
            highAsfPlayers[client.playerNum].initialMass = highAsfPlayers[client.playerNum].ent.mass;
...
void GT_ThinkRules()
{
    ...
    for ( int i = 0; i < maxClients; i++ )
    {
        ...
        if ( @ent == null )
            continue;
        if (@client == null || ent.isGhosting() )
            continue;

        highAsfPlayers[client.playerNum].think();
    }
...
void GT_SpawnGametype()
{
    ...
    for ( int i = 0; i < maxClients; i++ )
    {
        @highAsfPlayers[i].client = G_GetClient( i );
        @highAsfPlayers[i].ent = @highAsfPlayers[i].client.getEnt();
        highAsfPlayers[i].initialMass = highAsfPlayers[i].ent.mass;
    }
}
