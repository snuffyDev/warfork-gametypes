...
Cvar g_steal_momentum( "g_steal_momentum", "1", 0 );
Cvar g_steal_lookangle( "g_steal_lookangle", "1", 0 );
    ...
    void playerKilled( Entity @target, Entity @attacker, Entity @inflictor )
    {
        ...
        if ( @target != null && @target.client != null && @attacker != null && @attacker.client != null )
        {
            ...
            if ( attackerCount >= 1 && targetCount >= 1 ) {
                // move the attackers's position/angle/velocity so that the attacker's position/angle/velocity is updated
                // to match the target's
                attacker.origin = target.origin;
                if (g_steal_momentum.integer == 1) {
                    attacker.angles = target.angles;
                }
                if (g_steal_momentum.integer == 1) {
                    attacker.velocity = target.velocity;
                }

                // add a teleportation effect
                attacker.teleportEffect(false);
            }
            if ( attackerCount == 1 && targetCount == 1 )
            {
...
bool GT_Command( Client @client, const String &cmdString, const String &argsString, int argc )
{
    ...
    {
        // check if the voted value is valid for: allow_collision, steal_momentum, steal_lookangle
        String cvarname = argsString.getToken( 0 );
        String cvarvalue = argsString.getToken( 1 );

        if ( cvarname == "allow_collision" )
        {
            if ( cvarvalue == "0" || cvarvalue == "1" )
                return true;
        }
        else if ( cvarname == "steal_momentum" || cvarname == "steal_lookangle" )
        {
            if ( cvarvalue == "0" || cvarvalue == "1" )
                return true;
        }
    ...
    {
        String votestring = argsString.getToken( 0 );
        String votevalue = argsString.getToken( 1 );
        if ( votestring == "allow_collision" )
        {
            g_allow_collision.set( votevalue.toInt() );
            return true;
        }
        else if ( votestring == "steal_momentum" )
        {
            g_steal_momentum.set( votevalue.toInt() );
            return true;
        }
        else if ( votestring == "steal_lookangle" )
        {
            g_steal_lookangle.set( votevalue.toInt() );
            return true;
        }
    }
...
void GT_PlayerRespawn( Entity @ent, int old_team, int new_team )
{
    ...
    if ( g_allow_collision.integer != 0 )
...
void GT_InitGametype()
{
    ...
    // add callvotes
    G_RegisterCallvote( "steal_momentum", "1 or 0", "integer", "Toggle the 'steal_momentum' modifier, which allows players to steal the momentum of the player they kill.\n\nCurrent: " + g_steal_momentum.integer + "\n");
    G_RegisterCallvote( "steal_lookangle", "1 or 0", "integer", "Toggle the 'steal_lookangle' modifier, which allows players to steal the look angle of the player they kill.\n\nCurrent: " + g_steal_lookangle.integer + "\n");
    G_RegisterCallvote( "allow_collision", "1 or 0", "integer", "Toggle the 'allow_collision' modifier, which allows players to collide with each other.\n\nCurrent: " + g_allow_collision.integer + "\n");
